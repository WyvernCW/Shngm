...
export const API = {
    ...
    mwd: {
        async getLatest(page = 1, priority = PRIORITY.HIGH) {
            return fetchWithRetry(`/api/manhwadesu/series?page=${page}`, {}, 2, 1000, priority);
        },
        async getDetail(slug, priority = PRIORITY.HIGH) {
            ...
            const data = await fetchWithRetry(`/api/manhwadesu/detail?slug=${clean}`, {}, 2, 1000, priority);
        ...
        async getPages(slug, chapter, priority = PRIORITY.HIGH) {
            ...
            const res = await fetchWithRetry(`/api/manhwadesu/pages?slug=${cleanSlug}&chapter=${cleanCh}`, {}, 2, 1000, priority);
            const images = (res?.data || []).map(img => `/api/image-proxy?url=${encodeURIComponent(img)}`);
            return { data: { chapter: { data: images }, base_url: '', path: '' } };
        },
        async search(q, priority = PRIORITY.HIGH) {
            const cleanQ = String(q || '').trim();
            if (!cleanQ) return { data: [] };
            return fetchWithRetry(`/api/manhwadesu/search?q=${encodeURIComponent(cleanQ)}`, {}, 2, 1000, priority);
        }
    },

    cos: {
        async getLatest(page = 1, priority = PRIORITY.HIGH) {
            return fetchWithRetry(`/api/cosmicscans/series?page=${page}`, {}, 2, 1000, priority);
        },
        async getDetail(slug, priority = PRIORITY.HIGH) {
            const clean = String(slug || '').trim().replace(/^['"`]+|['"`]+$/g, '');
            if (!clean || clean === 'undefined' || clean === 'null' || clean === '') {
            ...
            const data = await fetchWithRetry(`/api/cosmicscans/detail?slug=${clean}`, {}, 2, 1000, priority);
        ...
        async getPages(slug, chapter, priority = PRIORITY.HIGH) {
            ...
            const res = await fetchWithRetry(`/api/cosmicscans/pages?slug=${cleanSlug}&chapter=${cleanCh}`, {}, 2, 1000, priority);
            const images = (res?.data || []).map(img => `/api/image-proxy?url=${encodeURIComponent(img)}`);
            return { data: { chapter: { data: images }, base_url: '', path: '' } };
        },
        async search(q, priority = PRIORITY.HIGH) {
            const cleanQ = String(q || '').trim();
            if (!cleanQ) return { data: [] };
            return fetchWithRetry(`/api/cosmicscans/search?q=${encodeURIComponent(cleanQ)}`, {}, 2, 1000, priority);
        }
    },
